...
```{r}
# YOUR CODE HERE
out <- prepDocuments(temp$documents, temp$vocab, temp$meta, lower.thresh = 9)
...
# mod_5 <- stm(docs, vocab, K = 5, prevalence = ~ REGION + s(YEAR), data = meta, seed = 15, max.em.its = 15)
# 
# mod_40 <- stm(docs, vocab, K = 40, prevalence = ~ REGION + s(YEAR), data = meta, seed = 15, max.em.its = 15)
```
...
# Shows the terms associated with each of the 15 models. Helpful for interpreting what labels to create
labelTopics(model)

# Shows most representative documents for each topic
findThoughts(model, texts = meta$TITLE, n=2,topics = 1:15)

# Store your hand labels below.
labels_hand = c("Business", "Sports", "Healthcare", 
                "Fashion", "Art", "International Organizations", 
                "Sexual Violence", "War", "Women's Rights", 
                "Politics", "Education", "Human Interest",
                "Family", "Religion", "Reproductive Health")
```
...
# First prepare estimateEffect. Look at all topics compared to covariate of time
prep2 <- estimateEffect(formula = 1:15 ~ YEAR, model, meta = meta, uncertainty = "Global", documents=docs)

# Plot topic 9 over time
plot.estimateEffect(prep2, covariate = "YEAR", method = "continuous", topics = 9,
                    model = model)
