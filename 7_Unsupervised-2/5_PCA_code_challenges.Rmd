...
# New dfm. Minimal preprocessing--removing punctuation and stopwords
mach_dfm2 <- 
  tokens(mach_corp, remove_punct = T) %>% 
  tokens_tolower(keep_acronyms = F) %>%
  tokens_remove(pattern = stopwords("en")) %>%
  dfm()

# Fit wordfish model
mod <- textmodel_wordfish(mach_dfm2, sparse = T)

# Summarize model
summary(mod)

# Plot word positions
textplot_scale1d(mod, margin = "features", highlighted = c("ruler", "prince", "king", "kill", "state"))

```

### Wordfish with Irish Debates

```{r}
# Preprocess and fit model
mod_ir <- data_corpus_irishbudget2010 %>% 
...
# Visualize
textplot_scale1d(mod_ir)

# Group by party
textplot_scale1d(mod_ir, groups = data_corpus_irishbudget2010$party)

# Visualize key terms
textplot_scale1d(mod_ir, margin = "features", highlighted = c("government", "global", "children", 
...
# Fit 
mod_un <- textmodel_wordfish(un_dfm)

# Visualize
textplot_scale1d(mod_un)

# Visualize key terms
textplot_scale1d(mod_un, margin = "features",
                 highlighted = c("development", "climate", "island", "occupation", "conservation", "international", "palestine", "ocean"))
